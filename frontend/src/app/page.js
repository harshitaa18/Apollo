...
export default function HomePage() {
  ...
  // This function will handle changes in filters
  const handleFilterChange = (updatedFilter) => {
    if (updatedFilter.name === "clearAll") {
      ...
      // Handle logic to get doctors near the user
      setFilters((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        const res = await fetch('https://apollob.onrender.com/api/doctors');
  ...
  const closeForm = () => {
    setShowAddDoctorForm(false); // Close the modal
  };
  ...
  return (
    <div className="p-8  min-h-screen">
      ...
      {/* Conditionally render AddDoctor form with blur effect */}
      {showAddDoctorForm && (
        <>
          {/* Modal Backdrop (blur + light overlay) */}
